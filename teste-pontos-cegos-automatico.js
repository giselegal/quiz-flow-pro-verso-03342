...
function testarEditorUnified() {
  ...
  try {
    const result = execSync('curl -s http://localhost:8082/editor-unified | grep -c "editor-unified"', { 
      encoding: 'utf8',
      timeout: 10000 
    });
...
function analisarConfiguracaoDnd() {
  ...
  try {
    ...
    resultados.dndContext = editorUnified.includes('DndContext') && editorUnified.includes('DragEndEvent');
    resultados.sensors = editorUnified.includes('PointerSensor') && editorUnified.includes('useSensors');
