...
export function useEditorValidation() {
  const { validateField, getFieldErrors, clearFieldErrors, hasErrors, allErrors } =
    useValidationContext();

  const validateBlock = useCallback(
    (blockId: string, values: Record<string, unknown>, validations: BlockValidations) => {
  ...
  const validateStep = useCallback(
    (
      stepId: string,
      blocks: Array<{ id: string; type: string; values: Record<string, unknown> }>,
      validations: Record<string, BlockValidations>
    ) => {
