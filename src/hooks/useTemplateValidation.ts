...
export function useTemplateValidation(): ValidationService {
  const { validateField, validateBlock, getFieldErrors, hasErrors } = useEditorFieldValidation();
    ...
    (
      ...
      switch (stepId) {
        case 'step-1':
          ...
          hasRequiredBlocks =
            blocks.some(b => b.type === 'text-block') &&
  ...
  const validateTemplateField = useCallback(
    (blockId: string, fieldId: string, value: unknown, blockType: string) => {
