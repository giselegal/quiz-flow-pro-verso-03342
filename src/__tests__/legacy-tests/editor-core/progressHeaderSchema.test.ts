...
import { getBlockSchema, BasePropertySchema } from '@/components/editor/quiz/schema/blockSchema';

describe('progress-header schema', () => {
    it('define defaults principais', () => {
        const schema = getBlockSchema('progress-header');
        expect(schema).toBeTruthy();
        const props = Object.fromEntries(schema!.properties.map((p: BasePropertySchema) => [p.key, p]));
