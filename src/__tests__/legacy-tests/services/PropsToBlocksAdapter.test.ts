// src/tests/services/PropsToBlocksAdapter.test.ts
...
describe('PropsToBlocksAdapter', () => {
  ...
  test('offer props -> quiz-offer-cta-inline blocks', () => {
    const step = { id: 's20', type: 'offer', meta: { props: { offerMap: { o1: { title: 'Oferta 1', price: 10 } } } } };
    const converted = PropsToBlocksAdapter.applyPropsToBlocks(step);
    expect(converted.blocks.some((b: any) => b.type === 'quiz-offer-cta-inline')).toBe(true);
  });
});
