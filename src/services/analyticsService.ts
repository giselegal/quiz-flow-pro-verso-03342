...
export interface AnalyticsEvent {
  ...
  user_id?: string;
  session_id: string;
  event_type: 'quiz_started' | 'question_answered' | 'quiz_completed' | 'page_viewed' | 'button_clicked' | 'form_submitted';
...
export class AnalyticsService {
  ...
  async trackQuestionAnswer(quizId: string, questionId: string, answer: any, userId?: string): Promise<void> {
    await this.trackEvent({
      quiz_id: quizId,
      user_id: userId,
  ...
  async trackButtonClick(quizId: string, buttonId: string, buttonText: string, userId?: string): Promise<void> {
    await this.trackEvent({
      quiz_id: quizId,
      user_id: userId,
  ...
  private calculateMetrics(events: AnalyticsEvent[]): Omit<AnalyticsMetrics, 'quiz_id' | 'last_updated'> {
    ...
    if (startEvents.length > 0 && completionEvents.length > 0) {
      const times = completionEvents.map(completion => {
