...
export function EditorProPageSimple() {
  ...
  // Handlers para funcionalidades IA com integração real
...
// Wrapper components with AI integration
function AIStepGeneratorWrapper({ onStepsGenerated, onClose }: { onStepsGenerated: (steps: any[]) => void; onClose: () => void }) {
  const { actions } = useSimpleBuilder();
  
  const handleStepsGenerated = (steps: any[]) => {
    // Apply steps directly to the builder
...
function TemplatesIASidebarWrapper({ onSelectTemplate, onClose }: { onSelectTemplate: (template: FunnelTemplate) => void; onClose: () => void }) {
  const { actions } = useSimpleBuilder();
  
  const handleSelectTemplate = async (template: FunnelTemplate) => {
    // Load template directly into the builder
    if (template.id) {
