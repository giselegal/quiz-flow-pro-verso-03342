...
// ================= BLOCOS DE PERGUNTA (Compatibilidade com tipos do template) =================
export const QuestionProgressInlineBlock = defineBlock({
    id: 'question-progress',
    label: 'Pergunta: Progresso',
    category: 'question',
    schema: z.object({
    ...
    defaultConfig: { showPercent: true },
    render: ({ config, state }) => {
        const current = Math.max(1, config.current ?? state?.currentStep ?? 1);
        const total = Math.max(current, config.total ?? state?.totalSteps ?? 10);
        ...
        return (
            <div className="w-full space-y-1">
                <div className="flex items-center justify-between text-[11px] text-slate-600">
                    ...
                    {config.showPercent && <span>{pct}%</span>}
                </div>
...
export const QuestionNumberInlineBlock = defineBlock({
    id: 'question-number',
    label: 'Pergunta: Número',
    category: 'question',
    schema: z.object({
        prefix: z.string().default('Pergunta'),
        current: z.number().int().positive().optional(),
        total: z.number().int().positive().optional()
    }),
    defaultConfig: { prefix: 'Pergunta' },
    render: ({ config, state }) => {
        const current = config.current ?? state?.currentStep ?? undefined;
        const total = config.total ?? state?.totalSteps ?? undefined;
        return (
            <div className="text-xs font-semibold tracking-wide uppercase text-slate-600">
                {config.prefix}
                {current != null && total != null ? ` ${current} de ${total}` : ''}
            </div>
        );
    }
});

export const QuestionTextInlineBlock = defineBlock({
    id: 'question-text',
    label: 'Pergunta: Texto',
    category: 'question',
    schema: z.object({ text: z.string().default('Sua pergunta aqui') }),
    defaultConfig: { text: 'Sua pergunta aqui' },
    render: ({ config }) => (
        <h3 className="text-xl font-bold text-primary leading-snug">{config.text}</h3>
    )
});

export const QuestionInstructionsInlineBlock = defineBlock({
    id: 'question-instructions',
    label: 'Pergunta: Instruções',
    category: 'question',
    schema: z.object({ text: z.string().default('Selecione uma opção para continuar.') }),
    defaultConfig: { text: 'Selecione uma opção para continuar.' },
    render: ({ config }) => (
        <p className="text-xs opacity-70">{config.text}</p>
    )
});

export const OptionsGridInlineBlock = defineBlock({
    id: 'options-grid',
    label: 'Pergunta: Grid de Opções',
    category: 'question',
    schema: z.object({
        requiredSelections: z.number().int().positive().default(1),
        options: z.array(z.object({ id: z.string(), text: z.string(), image: z.string().optional() })).default([])
    }),
    defaultConfig: { requiredSelections: 1, options: [] },
    render: ({ config, state }) => {
        const [answers, setAnswers] = React.useState<string[]>([]);
        const required = config.requiredSelections ?? 1;
        const toggle = (optId: string) => {
            setAnswers(prev => {
                const selected = prev.includes(optId);
                let next = selected ? prev.filter(i => i !== optId) : [...prev, optId];
                if (next.length > required) next = next.slice(0, required);
                if (state?.onAnswersChange) state.onAnswersChange(next);
                if (next.length === required && state?.onComplete) {
                    setTimeout(() => state.onComplete(), 200);
                }
                return next;
            });
        };
        ...
        return (
            <div className={`grid ${hasImages ? 'grid-cols-2 gap-3' : 'grid-cols-1 gap-2'}`}>
                {options.map(opt => {
                    const active = answers.includes(opt.id);
                    return (
                        <button
...
export const QuestionNavigationInlineBlock = defineBlock({
    id: 'question-navigation',
    label: 'Pergunta: Navegação',
    category: 'question',
    schema: z.object({
        showBack: z.boolean().default(true),
        backLabel: z.string().default('Voltar'),
        nextLabel: z.string().default('Próxima')
    }),
    defaultConfig: { showBack: true, backLabel: 'Voltar', nextLabel: 'Próxima' },
    render: ({ config, state }) => (
        <div className="flex items-center justify-between gap-2 mt-2">
            {config.showBack ? (
                <button
                    type="button"
                    onClick={() => state?.onPrev && state.onPrev()}
            ...
            <button
                type="button"
                onClick={() => state?.onComplete && state.onComplete()}
...
export const EXTENDED_BLOCK_DEFINITIONS: BlockDefinition<any>[] = [
    ...
    // Compatibilidade com blocos de pergunta (templates normalizados)
    QuestionProgressInlineBlock,
    QuestionNumberInlineBlock,
    QuestionTextInlineBlock,
    QuestionInstructionsInlineBlock,
    OptionsGridInlineBlock,
    QuestionNavigationInlineBlock,
