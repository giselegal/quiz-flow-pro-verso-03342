...
export const ScrollSyncProvider: React.FC<ScrollSyncProviderProps> = ({ children }) => {
  ...
  const syncScroll = useCallback((source: 'canvas' | 'components' | 'properties', scrollTop: number) => {
    ...
    if (source !== 'components' && componentsScrollRef.current) {
      const componentsMaxScroll = componentsScrollRef.current.scrollHeight - componentsScrollRef.current.clientHeight;
      const componentsTargetScroll = Math.max(0, Math.min(componentsMaxScroll, scrollRatio * componentsMaxScroll));
      componentsScrollRef.current.scrollTop = componentsTargetScroll;
    }

    if (source !== 'properties' && propertiesScrollRef.current) {
      const propertiesMaxScroll = propertiesScrollRef.current.scrollHeight - propertiesScrollRef.current.clientHeight;
      const propertiesTargetScroll = Math.max(0, Math.min(propertiesMaxScroll, scrollRatio * propertiesMaxScroll));
