...
describe('Schema resolution preference', () => {
  ...
  it('should still have a legacy fallback for compatibility (when needed)', () => {
    // O fallback não precisa existir para todos, mas o getBlockDefinition deve ser chamável
    const def = getBlockDefinition('non-existent-type');
