...
export function calculateQuizResult(answers: QuizAnswer[], questions: QuizQuestion[]): QuizResult {
  ...
  return {
    primaryStyle: {
      ...
      percentage: Math.round(((styleScores[predominantStyle] || 0) / Object.values(styleScores).reduce((a, b) => a + b, 1)) * 100),
    ...
    secondaryStyles: complementaryStyles.map((style, index) => ({
      ...
      percentage: Math.round(((styleScores[style] || 0) / Object.values(styleScores).reduce((a, b) => a + b, 1)) * 100),
      style,
