...
export const templateValidations: Record<string, EditorValidations> = {
  ...
  'form-block': {
    ...
    submitButtonText: [{ type: 'required', properties: {} }],
  },
    ...
    columns: [
      {
        type: 'custom',
        properties: {
          validator: (value: unknown) => typeof value === 'number' && value >= 1 && value <= 4,
    ...
    timeLimit: [
      {
        type: 'custom',
        properties: {
          validator: (value: unknown) => typeof value === 'number' && value >= 30 && value <= 3600,
    ...
    currentStep: [
      {
        type: 'custom',
        properties: {
          validator: (value: unknown) => typeof value === 'number' && value >= 1,
