...
export default ContextComposer;
    ...
    constructor(initialSlices: Record<string, ContextSlice> = {}) {
        Object.entries(initialSlices).forEach(([name, slice]) => {
    ...
    getSlice<T>(sliceName: string): T {
        return this.slices.get(sliceName) as T || {} as T;
    }

    updateSlice(sliceName: string, updates: Partial<ContextSlice>): void {
        const currentSlice = this.slices.get(sliceName) || {};
        const newSlice = { ...currentSlice, ...updates };

        this.slices.set(sliceName, newSlice);
        ...
        if (sliceSubscribers) {
            sliceSubscribers.forEach(callback => {
                try {
                    callback(newSlice);
                } catch (error) {
                    console.error(`Error in subscriber callback for slice ${sliceName}:`, error);
                }
            });
        }
    }
    ...
    subscribe(sliceName: string, callback: (value: any) => void): () => void {
        ...
        return () => {
            ...
            if (sliceSubscribers.size === 0) {
                this.subscribers.delete(sliceName);
            }
        };
    }
...
// 🧩 Componente principal ContextComposer
export const ContextComposer: React.FC<ContextComposerProps> = ({
    children,
    slices = {},
    debugMode = false,
    ...
    // Criar store uma única vez
    const store = useMemo(() => new ContextStoreImpl(slices), []);
    ...
    useEffect(() => {
        if (debugMode) {
                ...
                // 🎣 Hook para usar slice específica
...
export const useContextPerformance = () => {
    const context = useContext(ComposedContext);
        ...
        throw new Error('useContextPerformance must be used within ContextComposer');
                    ...
                    // 🔧 Utilidade para compor múltiplos providers
    ...
    return providers.reduce(
        (AccumulatedProviders, CurrentProvider) => {
            return ({children}: {children: ReactNode }) => (
                        ...
                        export default ContextComposer;NTEXT COMPOSITION SYSTEM - FASE 1
                        *
