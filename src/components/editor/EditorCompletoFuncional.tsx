...
const EditorCompletoFuncional: React.FC = () => {
  ...
  const adicionarPergunta = useCallback((tipo: Question['tipo']) => {
    const novaPergunta: Question = {
      id: 'pergunta-' + Date.now(),
      tipo,
      pergunta: 'Nova pergunta',
      pontos: 10,
      ...(tipo === 'multipla-escolha' && { opcoes: ['Opção 1', 'Opção 2', 'Opção 3', 'Opção 4'] }),
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '20px', flexWrap: modoPreview ? 'wrap' : 'nowrap' }}>
        ...
        {!modoPreview && (
          <div style={{
            ...
            <h2 style={{ margin: '0 0 20px 0', color: '#333' }}>Perguntas ({quiz.perguntas.length})</h2>
            
            {/* Botões para adicionar pergunta */}
            <div style={{ marginBottom: '20px' }}>
              <p style={{ margin: '0 0 10px 0', fontWeight: 'bold', color: '#666' }}>Adicionar pergunta:</p>
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                <button
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                        <span style={{
                          background: pergunta.tipo === 'multipla-escolha' ? '#007bff' : 
                                     pergunta.tipo === 'verdadeiro-falso' ? '#17a2b8' : '#6c757d',
                          color: 'white',
                          padding: '2px 8px',
                          borderRadius: '4px',
                          fontSize: '12px',
          ...
          {modoPreview ? (
            ...
            <div>
              ...
              <div style={{
                ...
                <div style={{ marginBottom: '20px' }}>
                  <strong style={{ color: '#495057' }}>Total de perguntas: {quiz.perguntas.length}</strong>
                </div>
            ...
            <div>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <input
                  ...
                  onChange={(e) => atualizarPergunta(perguntaAtiva, 'pontos', parseInt(e.target.value) || 0)}
                  style={{
                    width: '100px',
                    padding: '8px 12px',
                    border: '1px solid #ced4da',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '30px', flexWrap: 'wrap' }}>
          ...
          <div>
            <strong style={{ color: '#28a745' }}>🎯 Pontos Total: {quiz.perguntas.reduce((sum, p) => sum + p.pontos, 0)}</strong>
          </div>
          <div>
            <strong style={{ color: '#6f42c1' }}>⏱️ Tempo Estimado: {Math.max(1, quiz.perguntas.length * 2)} min</strong>
          </div>
        </div>
      </div>
    </div>
