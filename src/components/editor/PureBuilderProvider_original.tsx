    ...
    try {
        ...
        if (safeTemplate === 'quiz21StepsComplete') {
            ...
            try {
                ...
                // Converter template local para o formato esperado
                Object.keys(QUIZ_STYLE_21_STEPS_TEMPLATE).forEach(stepKey => {
                    const templateBlocks = QUIZ_STYLE_21_STEPS_TEMPLATE[stepKey];
                    stepBlocks[stepKey] = templateBlocks || [];
                    console.log(`✅ Loaded local ${stepKey}: ${stepBlocks[stepKey].length} blocos`);
                });
            } catch (error) {
                console.error('❌ Erro ao carregar template local:', error);
                // Fallback para template vazio
            ...
            // ✅ TEMPLATE LOADING PARALELO - Para outros templates
            try {
