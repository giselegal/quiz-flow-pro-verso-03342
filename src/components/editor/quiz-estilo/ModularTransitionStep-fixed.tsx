...
export default function ModularTransitionStep({
    ...
    React.useEffect(() => {
        if (blocks.length === 0 && editor?.actions?.ensureStepLoaded) {
            console.log(`🔄 [ModularTransitionStep] Auto-loading ${stepKey}`);
    ...
    React.useEffect(() => {
        const delay = Number(data?.navigation?.autoAdvanceDelay ?? data?.metadata?.autoAdvanceDelay ?? 3000);
        const shouldAutoAdvance = (!!onComplete) && (enableAutoAdvance || !isEditable);
        if (shouldAutoAdvance) {
            const timer = setTimeout(() => onComplete(), isFinite(delay) ? delay : 3000);
