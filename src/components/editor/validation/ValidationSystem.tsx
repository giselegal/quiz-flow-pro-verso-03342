...
export const useValidation = () => {
  ...
  const validateField = useCallback((field: string, value: any, context?: FunnelValidationContext) => {
    const applicableRules = VALIDATION_RULES.filter(rule => 
      rule.field === field || rule.field.includes('[]') && field.includes(rule.field.replace('[]', ''))
    );
  ...
  const validateFunnel = useCallback(async (context: FunnelValidationContext) => {
    ...
    try {
      ...
      if (context.funnel) {
        ...
        if (context.funnel.seo) {
          ...
          allResults.push(...validateField('seo.description', context.funnel.seo.description, context));
        }
      }
      ...
      if (context.pages) {
        context.pages.forEach((page, index) => {
          ...
          allResults.push(...validateField(`pages[${index}].components`, page.components, context));
        });
      }
  ...
  const getValidationSummary = useMemo(() => {
    ...
    summary.score = maxPossibleIssues > 0 
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="bg-white p-4 rounded-lg border">
        <div className="flex items-center justify-between mb-2">
          ...
          <Badge variant={summary.score >= 70 ? "secondary" : summary.score >= 50 ? "outline" : "destructive"}>
            ...
            .map((result, index) => (
              <Alert key={index} className="bg-white">
                <div className="flex items-start gap-2">
                  ...
                  {result.severity === 'info' && <Info className="h-4 w-4 text-blue-500" />}
                  <div className="flex-1">
                    <AlertDescription className="text-sm">
