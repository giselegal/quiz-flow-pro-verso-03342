...
export const QuizIntroHeaderBlock: React.FC<QuizIntroHeaderBlockProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Logo */}
      {showLogo && logoUrl && (
        <div className="logo-container" style={{ marginBottom: showDecorativeBar && barPosition === 'bottom' ? '0' : '16px' }}>
          <img 
            src={logoUrl} 
            alt={logoAlt}
            style={logoStyle}
      ...
      {/* Modo de edição */}
      {isEditing && (
        <div className="editing-overlay" style={{
          position: 'absolute',
          inset: 0,
          ...
          <div style={{ fontSize: '12px', color: '#6B4F43' }}>
            {showLogo && showDecorativeBar ? 'Logo + Barra' : showLogo ? 'Apenas Logo' : showDecorativeBar ? 'Apenas Barra' : 'Personalizado'}
          </div>
        </div>
      )}
    </div>
