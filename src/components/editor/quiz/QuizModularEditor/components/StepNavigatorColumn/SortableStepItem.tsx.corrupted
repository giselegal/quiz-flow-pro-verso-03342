...
import {
  validateTemplateIntegrity as validateTemplateIntegrityFull,
  formatValidationResult,
} from '@/lib/utils/templateValidation';
// Loading context (provider + hook)
import {
  EditorLoadingProvider,
  useEditorLoading,
} from '@/contexts/EditorLoadingContext';
...
let MetricsPanel: React.LazyExoticComponent<React.ComponentType<any>> | null =
  null;
...
function QuizModularEditorInner(props: QuizModularEditorProps) {
  ...
  // Snapshot do último estado enfileirado no autosave (para evitar loops)
  ...
  const { queueSave: queueAutosave, flush: flushAutosave } = useQueuedAutosave({
    ...
    debounceMs: Number(
      (import.meta as any).env?.VITE_AUTO_SAVE_DELAY_MS ?? 2000
    ),
    maxRetries: 3,
    onUnsaved: (stepKey) => {
  ...
  useEffect(() => {
    try {
      ...
      appLogger.warn('[QuizModularEditor] Erro ao configurar funnel ativo:', {
        data: [error],
      });
    }
    ...
    return () => {
      try {
        ...
        appLogger.warn('[QuizModularEditor] Erro ao limpar funnel ativo:', {
          data: [error],
        });
      }
    };
  ...
  useEffect(() => {
    try {
      ...
      appLogger.warn(
        '[QuizModularEditor] Erro ao sincronizar funnel ativo do estado unificado:',
        { data: [error] }
      );
    }
  ...
  useEffect(() => {
    try {
      const progress = Number(
        (resourceMetadata as any)?.metadata?.progress ?? 1
      );
      if (progress < 1) {
  ...
  const handleSelectStep = useCallback(
    (key: string) => {
      ...
      if (tid) {
        (async () => {
          try {
            ...
            appLogger.warn(`⚠️ [G2] Erro ao carregar step ${key}:`, {
              data: [error],
            });
          }
        })();
      ...
      if (loadedTemplate?.steps?.length) {
        const index = loadedTemplate.steps.findIndex(
          (s: any) => s.id === key
        );
  ...
  useEffect(() => {
    try {
      ...
      appLogger.warn(
        '[QuizModularEditor] Erro ao restaurar layout de painéis:',
        { data: [error] }
      );
    }
  }, []);
  ...
  useEffect(() => {
    if (
      !props.templateId &&
      !loadedTemplate &&
      (!unifiedState.editor.currentStep || unifiedState.editor.currentStep < 1)
    ) {
      appLogger.info(
        '🎨 [QuizModularEditor] Modo livre - inicializando currentStep = 1'
      );
      setCurrentStep(1);
    }
  ...
  useEffect(() => {
    ...
    const stepBlocks = unifiedState.editor
      .stepBlocks as Record<string, Block[]>;
    const currentBlocks = stepBlocks[currentStepKey] || [];

    if (!currentBlocks || currentBlocks.length === 0) return;

    // Snapshot leve para evitar loop infinito de autosave
    ...
    if (lastAutosaveSnapshotRef.current === snapshot) {
      // Nada mudou desde o último autosave enfileirado
      return;
    }
  ...
  const normalizeOrder = useCallback(
    (list: Block[]) => list.map((b, idx) => ({ ...b, order: idx })),
    []
  );
  ...
  useEffect(() => {
    if (!props.templateId && !resourceId) {
      appLogger.info(
        '🎨 [QuizModularEditor] Modo canvas vazio - sem template'
      );
      return;
    }
    ...
    async function loadTemplateOptimized() {
      ...
      try {
        ...
        const tid = props.templateId ?? resourceId!;
        appLogger.info(
          `🔍 [QuizModularEditor] Carregando metadata do template: ${tid}`
        );

        // 🔥 FASE 2: Warmup de cache - prefetch steps iniciais (1, 2, 3)
        const { cacheManager } = await import('@/lib/cache/CacheManager');
        const { loadStepFromJson } = await import(
        ...
        const templateStepsResult =
          (await svc.steps?.list?.({ signal })) ?? { success: false };
        let stepsMeta: any[] = [];
        ...
        if (!signal.aborted) {
          appLogger.info(
            `✅ [QuizModularEditor] Metadata carregada: ${stepsMeta.length} steps`
          );
    ...
    async function runFullValidation(
      ...
      try {
        appLogger.info(
          `[G5] Iniciando validação completa: ${tid} (${stepCount} steps)`
        );
        ...
        if (!signal.aborted) {
          appLogger.error(
            '[G5] Erro ao validar integridade do template:',
            error
          );
        }
      }
    ...
    setCurrentStep,
    showToast,
    unifiedState.editor.currentStep,
  ]);
    ...
    critical.forEach((sid) => {
      queryClient
        .prefetchQuery({
          ...
          queryFn: async () => {
            const res = await templateService.getStep(
              sid,
              templateOrResource ?? undefined
            );
  ...
  useEffect(() => {
    ...
    async function ensureStepBlocks() {
      ...
      try {
        const svc: any = templateService;
        const result = await svc.getStep(
          stepId,
    ...
    try {
      ...
      neighborIds.forEach((nid) => {
        queryClient
          ...
          .catch((err) => {
            appLogger.warn(
              '[QuizModularEditor] prefetch neighbor failed',
              err
            );
          });
      });
  ...
  const handleDragEnd = useCallback(
    (event: any) => {
      ...
      const { draggedItem, overId, activeId } = result as {
        draggedItem: any;
        overId: any;
        activeId: any;
      };
      ...
      if (draggedItem?.type === 'library-item') {
        ...
        try {
          ...
          appLogger.error(
            '[DnD] Falha ao adicionar bloco da biblioteca, executando rollback',
            {
              error,
              blockType: draggedItem.libraryType,
            }
      ...
      if (
        ...
        const fromIndex = list.findIndex(
          (b) => String(b.id) === String(activeId)
        );
        const toIndex = list.findIndex(
          (b) => String(b.id) === String(overId)
        );
        if (fromIndex >= 0 && toIndex >= 0) {
          ...
          try {
            ...
            appLogger.error(
              '[DnD] Falha ao reordenar blocos, executando rollback',
              {
                error,
                fromIndex,
                toIndex,
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      try {
        ...
        appLogger.warn(
          '[QuizModularEditor] Erro ao salvar steps pendentes antes do snapshot:',
          { data: [error] }
        );
      }
      await saveFunnel();
  ...
  const handleReloadStep = useCallback(async () => {
    ...
    appLogger.info(
      `🔄 [QuizModularEditor] Recarregando step após erro: step-${stepIndex}`
    );
    ...
    try {
      ...
      try {
        ...
        appLogger.warn(
          '[QuizModularEditor] Erro ao invalidar queries do React Query:',
          { data: [error] }
        );
      }
      ...
      if (result.success && result.data) {
        setStepBlocks(stepIndex, result.data);
        appLogger.info(
          `✅ [QuizModularEditor] Step recarregado: ${result.data.length} blocos`
        );
      }
    } catch (error) {
    ...
    try {
      const data = {
        meta: {
          ...
          template:
            props.templateId || loadedTemplate?.name || resourceId || null,
        },
  ...
  const handleExportV3 = useCallback(() => {
    try {
      const stepsEntries = Object.entries(
        unifiedState.editor.stepBlocks || {}
      ) as Array<[string, any[]]>;
      ...
      for (const [indexStr, blocks] of stepsEntries) {
        ...
        const stepId = `step-${String(
          Number.isFinite(indexNum) ? indexNum : indexStr
        ).padStart(2, '0')}`;
  ...
  const handlePublish = useCallback(async () => {
    try {
      ...
      if (loadedTemplate) {
        ...
        appLogger.info(
          '[G5] Resultado da validação pré-publicação:',
          formattedResults
        );
      ...
      try {
        ...
        appLogger.warn(
          '[G42] Erro ao invalidar cache após publicação',
          cacheError
        );
      }
  ...
  const handleLoadTemplate = useCallback(async () => {
    ...
    try {
      ...
      appLogger.info(
        `🔍 [QuizModularEditor] Preparando template via botão (lazy): ${tid}`
      );
    ...
    async (template: any, stepId?: string) => {
      try {
        ...
        if (criticalErrors.length > 0) {
          ...
          throw new Error(
            `Template possui ${criticalErrors.length} erros críticos`
          );
        }
        ...
        appLogger.error(
          '[QuizModularEditor] Erro ao importar template:',
          error
        );
        showToast({
          type: 'error',
  ...
  return (
    ...
    >
      <div
        className="qm-editor flex flex-col h-screen bg-gray-50"
        data-editor="modular-enhanced"
      >
        {/* Header */}
        ...
        <PanelGroup
          ...
          onLayout={(sizes: number[]) => {
            try {
              ...
              appLogger.warn(
                '[QuizModularEditor] Falha ao processar blocos:',
                error
              );
            }
          }}
              ...
              >
                {isLoadingTemplate ? (
                  ...
                  >
                    <div
                      className={
                        isLoadingStep ? 'pointer-events-none opacity-50' : ''
                      }
                    >
                      <CanvasColumn
                        currentStepKey={currentStepKey}
                        ...
                        hasTemplate={Boolean(
                          loadedTemplate || props.templateId || resourceId
                        )}
                    ...
                    <PreviewPanel
                      ...
                      onStepChange={(sid: string | null) => {
                        ...
                        const num = match
                          ? parseInt(match[1], 10)
                          : safeCurrentStep;
                        if (Number.isFinite(num) && num !== safeCurrentStep)
                          setCurrentStep(num);
                      }}
                    />
                  </StepErrorBoundary>
        ...
        {import.meta.env.DEV && (
          <Suspense fallback={null}>
            <PerformanceMonitor
              selectedBlockId={selectedBlockId}
              selectedBlockType={
                blocks?.find((b) => b.id === selectedBlockId)?.type || null
              }
            />
          </Suspense>
        )}
