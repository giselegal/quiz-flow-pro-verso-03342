...
export const InteractiveQuizCanvas: React.FC<InteractiveQuizCanvasProps> = memo(({ 
  ...
  const handleQuizAnswer = useCallback((answer: {
    ...
    quizState.answerStrategicQuestion?.(questionId, selectedOptions.join(','), selectedOptions.length);

  }, [activeStageId, quizState]);

  // Calcular pontuação
  const calculateAndUpdateScores = useCallback((selectedOptions: string[], scoreValues: Record<string, number>) => {
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="debug-panel fixed bottom-4 right-4 bg-white/90 p-4 rounded-lg shadow-lg max-w-sm">
          ...
          <div className="text-xs space-y-1">
            <div><strong>Step:</strong> {activeStageId}</div>
            <div><strong>Blocks:</strong> {currentBlocks.length}</div>
            <div><strong>Answers:</strong> {quizAnswers.length}</div>
            <div><strong>Valid:</strong> {currentValidation?.isValid ? '✅' : '❌'}</div>
            <div><strong>Scores:</strong></div>
            <div className="ml-2">
              {Object.entries(scores).map(([category, score]) => (
                <div key={category}>{category}: {score}</div>
              ))}
            </div>
          </div>
        </div>
