...
export const ValidationMessages: React.FC<ValidationMessagesProps> = memo(({
  ...
  const interactedInvalidFields = invalidFields.filter(
    fieldId => hasInteractedWith[fieldId]
  );
...
export function useFieldValidation() {
  ...
  const updateValidation = React.useCallback((fieldId: string, isValid: boolean, fieldType?: string) => {
  ...
  const interactedErrorCount = React.useMemo(() => {
    return Object.entries(validationState)
      .filter(([fieldId, isValid]) => !isValid && hasInteractedWith[fieldId])
      .length;
