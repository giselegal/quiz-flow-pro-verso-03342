...
export const QUIZ_EXEMPLO_COMPLETO: Block[] = [
  ...
  },
  {
    ...
    content: {
      ...
      href: '#submit-quiz',
      className: 'bg-gradient-to-r from-purple-600 to-pink-600 text-white text-xl font-bold py-4 px-8 rounded-full hover:shadow-xl transition-all duration-300 mx-auto block w-fit'
    }
  }
];
...
export const QUIZ_RESULTADO_MAPPING = {
  extrovertido: {
    ...
    description: 'Pessoas com seu estilo têm facilidade para se conectar com outros, liderar conversas e criar energia positiva ao redor. Você se destaca em ambientes sociais e tem um talento natural para influenciar e inspirar.',
    characteristics: [
  ...
  contemplativo: {
    ...
    description: 'Seu estilo é marcado pela busca do conhecimento, auto-reflexão e uma abordagem ponderada da vida. Você prefere qualidade à quantidade, tanto em relacionamentos quanto em experiências.',
    characteristics: [
  ...
  criativo: {
    ...
    description: 'Sua criatividade e originalidade são suas maiores forças. Você tem uma visão única do mundo e não tem medo de experimentar e inovar em todas as áreas da vida.',
    characteristics: [
  ...
  classico: {
    ...
    description: 'Seu estilo é refinado e elegante. Você valoriza qualidade, tradição e tem um gosto apurado para o que é verdadeiramente belo e duradouro.',
    characteristics: [
  ...
  social: {
    ...
    description: 'Você tem um talento especial para entender as pessoas e criar harmonia em grupos. Sua empatia e habilidades sociais fazem de você um verdadeiro conectador.',
    characteristics: [
...
export function calcularResultadoQuiz(respostas: Record<string, any>): {
  ...
  const categoriaDominante = Object.entries(pontuacoes)
    .sort(([,a], [,b]) => b - a)[0]?.[0];

  const resultado = QUIZ_RESULTADO_MAPPING[categoriaDominante as keyof typeof QUIZ_RESULTADO_MAPPING] || QUIZ_RESULTADO_MAPPING.social;
