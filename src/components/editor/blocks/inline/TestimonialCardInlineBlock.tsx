...
const TestimonialCardInlineBlock: React.FC<BlockComponentProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Testimonial text */}
      <blockquote className="text-[#432818] text-sm leading-relaxed mb-4 font-medium">"{testimonial}"</blockquote>

      {/* Rating stars */}
      <div className="flex space-x-1 mb-4">
        {[1, 2, 3, 4, 5].map(star => (
          <span key={star} className={cn('text-lg', star <= rating ? 'text-yellow-400' : 'text-gray-300')}>
            ★
          </span>
        ))}
      </div>
      ...
      <div className="flex items-center space-x-3">
        <img src={avatar} alt={name} className="w-12 h-12 rounded-full object-cover border-2 border-[#B89B7A]/20" />
        <div>
