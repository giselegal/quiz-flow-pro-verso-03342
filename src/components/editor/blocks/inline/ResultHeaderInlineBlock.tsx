...
const ResultHeaderInlineBlock: React.FC<BlockComponentProps> = ({
  ...
  return (
    ...
    >
      ...
      {showDecorations && (
        <div className="flex justify-center mb-4 sm:mb-6">
          <div className="flex space-x-1">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="w-2 h-2 rounded-full bg-gradient-to-r from-[#B89B7A] to-[#A1835D] opacity-60" />
            ))}
          </div>
        </div>
      )}
      ...
      {/* Subtítulo */}
      <p className="text-base sm:text-lg md:text-xl opacity-80 font-medium" style={{ color: textColor }}>
        {subtitle}
      </p>
