...
const ImageBlock: React.FC<ImageBlockProps> = ({
  ...
  const containerStyle: React.CSSProperties = {
    ...
    textAlign: (content.textAlign as React.CSSProperties['textAlign']) || 'center',
  };

  const imageStyle: React.CSSProperties = {
    width: (content.width as string) || '100%',
    height: (content.height as string) || 'auto',
    objectFit: (content.objectFit as React.CSSProperties['objectFit']) || 'cover',
    display: 'block',
    margin: '0 auto',
    borderRadius: (content.borderRadius as string) || '0.5rem',
    boxShadow: (content.boxShadow as string) || 'none',
  };
    ...
    >
      {content.url ? (
        <img
          ...
          onError={e => {
            try {
              const w = parseInt(String((content.width as any) || 600), 10) || 600;
              const h = parseInt(String((content.height as any) || 400), 10) || 400;
              (e.currentTarget as HTMLImageElement).src = safePlaceholder(w, h, content.alt || 'Imagem');
            } catch { }
          }}
        />
      ) : (
