...
const FAQSectionBlock: React.FC<BlockComponentProps> = ({
  ...
  const updateQuestion = (id: string, field: 'question' | 'answer', value: string) => {
    const updatedQuestions = questions.map((q: any) => (q.id === id ? { ...q, [field]: value } : q));
          ...
          {questions.map((faq: any, index: number) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div className="flex-1 pr-4">
                    <InlineEditableText
                      ...
                      className={cn('text-lg font-semibold', `text-[${BRAND_COLORS.secondary.main}]`, 'w-full')}
                      fontWeight="semibold"
                      textAlign="left"
                    />
                  </div>
                ...
                {/* Answer Content */}
                {isOpen && (
                  <div className={cn('p-4 md:p-5 lg:p-6', 'bg-white border-t border-gray-100', ANIMATIONS.transition)}>
                    <InlineEditableText
                      value={faq.answer}
