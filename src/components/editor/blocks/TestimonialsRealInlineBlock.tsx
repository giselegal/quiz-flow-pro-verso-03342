...
const TestimonialsRealInlineBlock: React.FC<TestimonialsRealInlineBlockProps> = ({
  ...
  const renderTestimonial = (testimonial: any, index: number) => (
    <div key={index} className="bg-white p-4 md:p-6 rounded-lg shadow-sm border border-gray-100 h-full flex flex-col">
      {/* Quote Icon */}
      <Quote className="w-6 h-6 text-pink-400 mb-3 flex-shrink-0" />

      {/* Depoimento */}
      <p className="text-gray-700 text-sm md:text-base mb-4 flex-grow leading-relaxed">"{testimonial.text}"</p>

      {/* Rating */}
      {showRating && <div className="flex items-center gap-1 mb-3">{renderStars(testimonial.rating || 5)}</div>}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {testimonial.occupation && <p className="text-gray-600 text-xs">{testimonial.occupation}</p>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      {/* Título */}
      <div className="mb-6">
        <h3 className="text-xl md:text-2xl font-bold text-center text-gray-800 mb-2">{title}</h3>
      ...
      <div
        className={cn(
          'grid gap-4 md:gap-6',
          layout === 'grid' && testimonials.length > 1 ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1',
