...
export class StepComponentFactory {
    ...
    register(config: ComponentConfig): void {
        ...
        if (config.preload && 'preload' in config.component && typeof (config.component as any).preload === 'function') {
            (config.component as any).preload();
    ...
    async preload(type: string): Promise<void> {
        ...
        if (config && 'preload' in config.component && typeof (config.component as any).preload === 'function' && !this.preloadedComponents.has(type)) {
            await (config.component as any).preload();
