/**
 * 🚀 VIRTUALIZED STEP NAVIGATOR - Fase 5 Gargalos
 * 
 * Navegador de steps com virtualização para listas longas (>15 steps).
...
interface VirtualizedStepNavigatorProps {
  currentStep: number;
  totalSteps: number;
  steps: StepInfo[];
  ...
  isLoading?: boolean;
  className?: string;
  /** Threshold para ativar virtualização (default: 15) */
...
const ESTIMATED_STEP_HEIGHT = 80; // px
...
const StepItem = React.memo<StepItemProps>(({ step, isCurrent, onClick }) => {
  ...
  return (
    <button
      ...
      className={cn(
        'w-full p-3 mb-2 rounded-lg border transition-all duration-200 text-left',
        isCurrent
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="font-medium text-sm">Etapa {step.order}</span>
            <Badge variant="secondary" className="text-xs px-1.5 py-0.5">
              {typeConfig.label}
            </Badge>
          </div>

          <div className="text-xs text-muted-foreground truncate">{step.name}</div>

          <div className="flex items-center gap-2 mt-1">
            <span className="text-xs text-muted-foreground">{step.blocksCount} blocos</span>
            {step.isComplete && <CheckCircle className="w-3 h-3 text-green-600" />}
            {step.hasError && <AlertCircle className="w-3 h-3 text-red-600" />}
          </div>
        </div>
...
export const VirtualizedStepNavigator: React.FC<VirtualizedStepNavigatorProps> = ({
  currentStep,
  totalSteps,
  steps,
  onStepChange,
  ...
  const shouldVirtualize = steps.length > virtualizationThreshold;

  // Configurar virtualizador
  const rowVirtualizer = useVirtualizer({
    count: steps.length,
    getScrollElement: () => parentRef.current,
    estimateSize: () => ESTIMATED_STEP_HEIGHT,
    overscan: 5,
  ...
  const handlePrevious = useCallback(() => {
    if (canGoPrevious) onStepChange(currentStep - 1);
  ...
  const handleStepClick = useCallback(
    (stepNumber: number) => onStepChange(stepNumber),
    [onStepChange]
  );

  const handlePreviewToggle = useCallback(() => {
    onPreviewMode?.(!isPreviewMode);
  }, [isPreviewMode, onPreviewMode]);

  // Scroll para step atual quando muda
  React.useEffect(() => {
    if (shouldVirtualize && currentStep > 0) {
      const index = steps.findIndex(s => s.order === currentStep);
      if (index >= 0) {
        rowVirtualizer.scrollToIndex(index, { align: 'center', behavior: 'smooth' });
      }
    }
  }, [currentStep, shouldVirtualize, steps, rowVirtualizer]);

  if (isCollapsed) {
    return (
      <div className={cn('w-12 bg-background border-r border-border flex flex-col', className)}>
        <Button
          variant="ghost"
          size="sm"
    ...
    <div className={cn('w-80 bg-background border-r border-border flex flex-col', className)}>
      ...
      <div className="p-4 border-t border-border">
        <div className="text-xs text-muted-foreground space-y-1">
          <div>Total: {totalSteps} etapas {shouldVirtualize && '(virtualizado)'}</div>
...
export default VirtualizedStepNavigator;
