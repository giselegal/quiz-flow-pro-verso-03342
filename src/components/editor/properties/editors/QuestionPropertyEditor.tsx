...
export const QuestionPropertyEditor: React.FC<PropertyEditorProps> = ({
  ...
  const handlePropertyChange = useCallback((propertyName: string, value: any) => {
    ...
    const isValid = propertyName === 'text' ? !!value : 
                   block.properties?.text || propertyName !== 'text';
    onValidate?.(isValid);
        ...
        <div className="mt-6 p-4 bg-[#FAF9F7] rounded-lg border">
          ...
          <div className="space-y-3">
            ...
            <div className="mt-3 p-2 border border-dashed border-[#E5DDD5] rounded text-xs text-[#8B7355]">
              ...
              {block.properties?.type === 'multiple' && '☑️ Opções de múltipla escolha apareceriam aqui'}
