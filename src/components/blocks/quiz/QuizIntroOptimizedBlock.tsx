    ...
    const renderStylePreviewsCompact = () => (
      <div className="flex flex-wrap justify-center gap-3">
        {QUIZ_STYLES.map(style => (
          <div
            key={style.name}
            className="flex items-center space-x-2 px-3 py-2 bg-white/60 rounded-full"
          >
            <div
      ...
      return (
        <div className="space-y-4">
          <div className="flex items-center justify-center space-x-2">
            <Badge variant="outline" style={{ borderColor: primaryColor, color: textColor }}>
              Etapa 1 de 21
            </Badge>
            <Badge variant="outline" style={{ borderColor: primaryColor, color: textColor }}>
              ⏱️ 3-5 minutos
      ...
      <Card className="bg-white/90 backdrop-blur-sm max-w-lg mx-auto">
        <CardContent className="p-6 space-y-4">
          ...
          {inputValue && !isValidInput && (
            <p className="text-sm text-red-500">{requiredFieldMessage}</p>
          )}
      ...
      >
        <CardContent className="p-8 text-center">
          ...
          <p className="text-white/90 mb-6 max-w-xl mx-auto">
            Vamos começar com algumas perguntas simples sobre suas preferências e descobrir juntas
            qual estilo reflete sua verdadeira essência.
          </p>
          {renderForm()}
        </CardContent>
      </Card>
