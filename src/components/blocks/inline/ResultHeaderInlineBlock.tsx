    ...
    >
        ...
        <div className="flex justify-center mb-4 sm:mb-6">
          <div className="flex space-x-1">
            {[...Array(5)].map((_, i) => (
              <div
                key={i}
                className="w-2 h-2 rounded-full bg-gradient-to-r from-[#432818] to-[#432818] opacity-60"
              />
            ))}
          </div>
        </div>
      ...
      {showDecorations && (
        <div className="flex justify-center mt-4 sm:mt-6">
          <div className="w-24 sm:w-32 h-1 bg-gradient-to-r from-transparent via-[#432818] to-transparent rounded-full" />
        </div>
      )}
    </div>
  );
