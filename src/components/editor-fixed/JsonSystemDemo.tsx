    ...
    try {
      ...
      if (success) {
        ...
        blocks.forEach((block, index) => {
          log(`  └─ Bloco ${index + 1}: ${block.type} (${block.content?.content?.toString().slice(0, 30) || 'sem conteúdo'}...)`);
        });
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="stat-card bg-purple-50 p-4 rounded-lg">
            ...
            <div className="text-2xl font-bold text-purple-800">
              {demoStep === 'intro' ? '🏁 Pronto' :
               demoStep === 'loading' ? '⏳ Carregando' :
               demoStep === 'loaded' ? '✅ Carregado' :
               '💾 Exportado'}
            </div>
          </div>
        </div>
        ...
        <div className="blocks-display mb-8 bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="blocks-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {blocks.map((block, index) => (
              <div key={block.id} className="block-card bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                <div className="block-id text-xs text-gray-600 mb-2">
                  ID: {block.id}
                </div>
  ...
  return (
    <div className="minimal-example p-4 bg-white rounded shadow">
      ...
      <div className="mb-4 text-sm text-gray-600">
        Template: {jsonFeatures.currentTemplate?.name || 'Nenhum'} | 
