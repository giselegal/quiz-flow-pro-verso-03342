...
export const useEditorWithJson = (
  ...
  const loadStepTemplate = useCallback(async (stepNumber: number): Promise<boolean> => {
  ...
  const loadCustomTemplate = useCallback(async (templatePath: string): Promise<boolean> => {
    setIsLoadingTemplate(true);
    setTemplateError(null);
  ...
  const exportCurrentAsTemplate = useCallback((metadata?: Partial<JsonTemplate>): JsonTemplate => {
  ...
  const applyTemplateToEditor = useCallback((template: JsonTemplate): Block[] => {
    const blocks = applyTemplate(template);
    setCurrentTemplate(template);
