...
export const PropertiesPanelDebug: React.FC<PropertiesPanelDebugProps> = ({ title = "Painel Debug" }) => {
  const {
    selectedBlockId,
  ...
  return (
    <div className="p-4 bg-yellow-50 border-2 border-yellow-200 rounded-lg m-2">
          ...
          <div className="bg-white p-2 rounded border">
            ...
            <ul className="text-xs mt-1">
              {currentBlocks.map(block => (
                <li key={block.id} className={block.id === selectedBlockId ? 'font-bold text-blue-600' : ''}>
                  {block.id} - {block.type}
                </li>
              ))}
            </ul>
