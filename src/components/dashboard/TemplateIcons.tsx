...
export const OfferPageIcon: React.FC<{ className?: string }> = ({ className = "w-6 h-6" }) => (
  <div className={`${className} relative`}>
    <svg viewBox="0 0 24 24" fill="none" className="w-full h-full">
      ...
      {/* Documento base */}
      <path d="M6 2h12a2 2 0 0 1 2 2v16a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2z" fill="url(#offerGradient)" opacity="0.8"/>
      {/* Linhas de conteúdo */}
...
export const MarketingQuizIcon: React.FC<{ className?: string }> = ({ className = "w-6 h-6" }) => (
  <div className={`${className} relative`}>
    <svg viewBox="0 0 24 24" fill="none" className="w-full h-full">
      ...
      {/* Corpo */}
      <path d="M6 21v-2a4 4 0 0 1 4-4h4a4 4 0 0 1 4 4v2" stroke="url(#marketingGradient)" strokeWidth="2" fill="none"/>
      {/* Lâmpada de ideias */}
...
export const DigitalProductIcon: React.FC<{ className?: string }> = ({ className = "w-6 h-6" }) => (
  <div className={`${className} relative`}>
    <svg viewBox="0 0 24 24" fill="none" className="w-full h-full">
      ...
      {/* Coração digital */}
      <path d="M12 14l-2-2a2 2 0 1 1 2.83-2.83l0.17 0.17 0.17-0.17A2 2 0 1 1 14 11l-2 2z" fill="#B89B7A"/>
      {/* Base do monitor */}
...
export const EcommerceIcon: React.FC<{ className?: string }> = ({ className = "w-6 h-6" }) => (
  <div className={`${className} relative`}>
    <svg viewBox="0 0 24 24" fill="none" className="w-full h-full">
      ...
      {/* Carrinho de compras */}
      <path d="M3 3h2l1.68 8.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L22 5H6" stroke="url(#ecommerceGradient)" strokeWidth="2" fill="none"/>
      {/* Rodas do carrinho */}
...
export const BackgroundPattern: React.FC<{ variant?: 'primary' | 'secondary' | 'accent' }> = ({ 
  ...
  return (
    <>
      <div className={`absolute top-0 right-0 w-20 h-20 bg-gradient-to-br ${pattern.circle1} rounded-full -translate-y-4 translate-x-4 blur-sm`}></div>
      <div className={`absolute bottom-0 left-0 w-16 h-16 bg-gradient-to-tr ${pattern.circle2} rounded-full translate-y-4 -translate-x-4`}></div>
