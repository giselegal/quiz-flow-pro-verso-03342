...
const shouldUseGrid = (schemas: ModernPropSchema[], category: string) => {
  const fieldKeys = schemas.map(s => s.key);

  // Detectar layout 2x2
  if (category === 'layout' && fieldKeys.some(key =>
    ['layout', 'direction', 'columns', 'disposition'].includes(key))) {
    return 'grid-cols-2';
  }

  // Detectar grid 3x3 para cores
  if (category === 'style' && fieldKeys.filter(key =>
    key.includes('Color') || key.includes('color')).length >= 3) {
    return 'grid-cols-3';
  }

  // Grid 2x3 para outros estilos
  if (category === 'style' && fieldKeys.length >= 4) {
    return 'grid-cols-2';
  }
  ...
  return (
    <div className="space-y-3">
      {value.map((option, index) => (
        <div key={option.id || index} className="w-full h-auto py-3 border-b border-gray-200">
          <div className="w-full h-auto flex flex-col justify-start items-start gap-3">
            <div className="w-full h-auto flex flex-row items-center justify-start gap-3">
              {/* Drag Handle (igual ao exemplo Cakto) */}
              ...
              {/* Campo de texto com max-width 48 (12rem) igual ao Cakto */}
              ...
              {/* Controles de reordenação e edição */}
              <div className="flex gap-1">
                ...
                {/* Botão de edição (lápis) igual ao Cakto */}
      ...
      {/* Botão para adicionar opção (igual ao Cakto) */}
      <Button
        variant="ghost"
        onClick={addOption}
        className="w-full border-dashed border-2 border-gray-300 text-gray-600 hover:border-gray-400 hover:text-gray-700 h-10"
      >
              ...
              return (
                <Card key={category} className="border border-gray-200 shadow-sm">
                  ...
                  <CardContent className="pt-0 space-y-4">
                    {(() => {
                      const gridClass = shouldUseGrid(schemas, category);

                      if (gridClass) {
                        // Renderizar com grid otimizado
                        return (
                          <div className={`grid ${gridClass} gap-3`}>
                            {schemas.map((schema) => {
                              // Verificar condições dependsOn e when
                              if (schema.dependsOn) {
                        ...
                        // Renderização tradicional em coluna
