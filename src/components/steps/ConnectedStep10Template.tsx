// 🔗 CONNECTED STEP 10 TEMPLATE - Questão: "QUE TIPO DE ACESSÓRIOS VOCÊ GOSTA?"
// Nona questão: Acessórios (texto) - 3 seleções obrigatórias - Auto-avanço

import { useCallback } from 'react';
import { useQuizLogic } from '@/hooks/useQuizLogic';
import { COMPLETE_QUIZ_QUESTIONS } from '@/data/correctQuizQuestions';

export const ConnectedStep10Template = () => {
  const { answerQuestion, answers } = useQuizLogic();
  
  // 🎯 Buscar questão real dos dados (q9)
  const questionData = COMPLETE_QUIZ_QUESTIONS.find(q => q.id === 'q9') || COMPLETE_QUIZ_QUESTIONS[9];
  ...
  const handleOptionSelect = useCallback(async (optionIds: string[]) => {
    try {
      ...
      if (selectedOption) {
        ...
        console.log('✅ Connected Step10: Resposta salva via hooks', { 
          questionId: questionData.id, 
          selectedOptions: optionIds 
        });
      }
    } catch (error) {
      console.error('❌ Connected Step10: Erro ao salvar resposta', error);
    }
  ...
  return [
    ...
    // 🎯 OPÇÕES DE ACESSÓRIOS CONECTADAS (DADOS REAIS)
    {
...
export const getConnectedStep10Template = () => {
  return ConnectedStep10Template();
};

export default ConnectedStep10Template;