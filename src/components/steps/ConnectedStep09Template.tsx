// 🔗 CONNECTED STEP 09 TEMPLATE - Questão: "QUAL DESSES SAPATOS VOCÊ TEM OU MAIS GOSTA?"
// Oitava questão: Sapatos com imagens - 3 seleções obrigatórias - Auto-avanço

import { useCallback } from 'react';
import { useQuizLogic } from '@/hooks/useQuizLogic';
import { COMPLETE_QUIZ_QUESTIONS } from '@/data/correctQuizQuestions';

export const ConnectedStep09Template = () => {
  const { answerQuestion, answers } = useQuizLogic();
  
  // 🎯 Buscar questão real dos dados (q8)
  const questionData = COMPLETE_QUIZ_QUESTIONS.find(q => q.id === 'q8') || COMPLETE_QUIZ_QUESTIONS[8];
  ...
  const handleOptionSelect = useCallback(async (optionIds: string[]) => {
    try {
      ...
      if (selectedOption) {
        ...
        console.log('✅ Connected Step09: Resposta salva via hooks', { 
          questionId: questionData.id, 
          selectedOptions: optionIds 
        });
      }
    } catch (error) {
      console.error('❌ Connected Step09: Erro ao salvar resposta', error);
    }
  ...
  return [
    ...
    // 🎯 GRADE DE SAPATOS CONECTADA (DADOS REAIS)
    {
    ...
    // 🔘 BOTÃO CONECTADO
    {
      ...
      properties: {
        ...
        onClick: () => {
          console.log('🎯 Connected Step09: Navegando para próximo step');
        }
      },
    },
  ];
...
export const getConnectedStep09Template = () => {
  return ConnectedStep09Template();
};

export default ConnectedStep09Template;