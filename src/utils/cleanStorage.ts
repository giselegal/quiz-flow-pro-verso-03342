...
export const cleanEditorLocalStorage = () => {
  try {
    ...
    const quizQuestionsBefore = config.blocks.filter((block: any) => block.type === 'quiz-question').length;
    ...
    if (quizQuestionsBefore > 0) {
      console.log(`✅ Cleaned ${quizQuestionsBefore} invalid quiz-question blocks from localStorage`);
