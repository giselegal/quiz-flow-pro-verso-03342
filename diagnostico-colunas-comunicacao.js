...
if (sidebar && canvas) {
  ...
  const distance = Math.sqrt(
    Math.pow(canvasRect.left - sidebarRect.right, 2) + Math.pow(canvasRect.top - sidebarRect.top, 2)
  );
...
if (firstDraggable) {
  console.log(
    '   └── Primeiro draggable encontrado:',
    firstDraggable.getAttribute('data-dnd-kit-draggable-id')
  );

  // Verificar se tem event listeners
  const hasListeners =
    firstDraggable.onmousedown || firstDraggable.ontouchstart || firstDraggable.ondragstart;
...
scrollableElements.forEach((el, i) => {
  ...
  console.log(
    `       ${i + 1}. overflow: ${style.overflow}, element: ${el.className.slice(0, 30)}`
  );
});
