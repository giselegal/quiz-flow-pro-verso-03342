...
const OptimizedBlockRenderer = React.memo<OptimizedBlockRendererProps>(
  ({ block, isSelected = false, onClick, onPropertyChange, isPreviewing = false }) => {
    ...
    const FallbackComponent = useMemo(
      () => (
        <div className="p-4 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
          <p className="text-sm text-gray-600">Componente '{block.type}' não encontrado</p>
    ...
    const componentProps = useMemo(
      ...
      [block, processedProperties, isSelected, isPreviewing, handleClick, onPropertyChange]
    );
