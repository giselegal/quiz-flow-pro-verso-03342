...
if (results.dynamicRenderer) {
  const etapas21 = [
    { etapa: 1, component: 'quiz-intro-etapa-1', desc: 'Introdução/Landing Page' },
    { etapa: '2-11', component: 'quiz-questao-principal', desc: 'Questões Principais (10 questões)' },
    { etapa: 12, component: 'quiz-transicao-principal', desc: 'Transição Meio' },
    { etapa: '13-18', component: 'quiz-questao-estrategica', desc: 'Questões Estratégicas (6 questões)' },
    { etapa: 19, component: 'quiz-transicao-final', desc: 'Transição Final' },
    { etapa: 20, component: 'quiz-resultado-completo', desc: 'Resultado Personalizado' },
    { etapa: 21, component: 'quiz-oferta-especial', desc: 'Oferta de Conversão' }
  ];
...
if (results.quizQuestionBlock) {
  const logicFeatures = [
    { feature: 'selectedOptions', regex: /selectedOptions.*useState/g, desc: 'Estado de seleções' },
    { feature: 'handleOptionClick', regex: /handleOptionClick.*=>/g, desc: 'Lógica de clique em opções' },
    { feature: 'isMultipleChoice', regex: /isMultipleChoice/g, desc: 'Suporte a múltipla escolha' },
    { feature: 'onOptionSelect', regex: /onOptionSelect.*\(/g, desc: 'Callback de seleção' },
    { feature: 'autoProceed', regex: /autoProceed/g, desc: 'Avanço automático' },
    { feature: 'validation', regex: /isRequired/g, desc: 'Validação de campos' }
  ];
...
if (results.dynamicRenderer) {
  const resultFeatures = [
    ...
    { feature: 'Características Personalizadas', regex: /Star.*CheckCircle.*Award/g },
...
if (results.dynamicRenderer) {
  const etapasComponents = [
    'quiz-intro-etapa-1', 'quiz-questao-principal', 'quiz-transicao-principal',
    'quiz-questao-estrategica', 'quiz-transicao-final', 'quiz-resultado-completo',
...
if (results.quizQuestionBlock) {
  const logicFeatures = ['selectedOptions', 'handleOptionClick', 'isMultipleChoice', 'onOptionSelect', 'autoProceed', 'isRequired'];
...
console.log(`🎯 Etapas do Funil: ${etapasImplementadas}/${totalEtapas} (${Math.round(etapasImplementadas/totalEtapas*100)}%)`);
console.log(`⚙️  Lógica de Quiz: ${logicaImplementada}/${totalLogica} (${Math.round(logicaImplementada/totalLogica*100)}%)`);
