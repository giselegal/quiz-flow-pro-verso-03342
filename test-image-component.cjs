...
try {
  ...
  const fs = require('fs');
  const path = '/workspaces/quiz-quest-challenge-verse/src/components/blocks/inline/ImageDisplayInlineBlock.clean.tsx';
  ...
  if (fs.existsSync(path)) {
    ...
    console.log(`   ${hasImageDisplayInlineBlockClean ? '✅' : '❌'} Nome do componente: ${hasImageDisplayInlineBlockClean}`);
