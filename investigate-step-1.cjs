...
async function testConsistency(jsonTemplate, tsxInfo) {
  ...
  console.log(
    `   📦 Form Container - JSON: ${jsonHasFormContainer ? '✅' : '❌'} | TSX: ${tsxInfo.hasFormContainer ? '✅' : '❌'}`
  );
  console.log(
    `   🏷️ Quiz Intro Header - JSON: ${jsonHasIntroHeader ? '✅' : '❌'} | TSX: ${tsxInfo.hasQuizIntroHeader ? '✅' : '❌'}`
  );
    ...
    global.fetch = async url => {
      if (url === '/templates/step-01-template.json') {
        const jsonContent = fs.readFileSync(
          path.join(__dirname, 'public/templates/step-01-template.json'),
          'utf8'
        );
        return {
          ok: true,
...
async function testComponentValidation() {
  ...
  try {
    ...
    console.log(
      `   📊 Cobertura de componentes: ${componentCoverage}/5 (${Math.round((componentCoverage / 5) * 100)}%)`
    );
...
async function runFullInvestigation() {
  ...
  if (allTestsPassing) {
    ...
    if (componentValidation?.componentCoverage < 4)
      console.log('   - Completar registro de componentes');
  }
}
