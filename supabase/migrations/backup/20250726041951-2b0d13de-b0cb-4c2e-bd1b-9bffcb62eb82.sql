...
-- Corrige comparação entre text e uuid com cast explícito
...
ON public.funnels 
FOR SELECT 
USING (auth.uid()::text = user_id AND auth.role() = 'authenticated');
...
ON public.funnels 
FOR INSERT 
WITH CHECK (auth.uid()::text = user_id AND auth.role() = 'authenticated');
