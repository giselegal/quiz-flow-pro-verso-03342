  ...
  registries.forEach(registryPath => {
    if (fs.existsSync(registryPath)) {
      ...
      if (content) {
        ...
        console.log(`   📤 Tem exports: ${registryAnalysis[registryPath].hasExport ? 'SIM' : 'NÃO'}`);
        console.log(`   🧩 Tem componentes: ${registryAnalysis[registryPath].hasComponents ? 'SIM' : 'NÃO'}`);
      }
    } else {
...
function analyzePropertiesHooks() {
  ...
  propertiesHooks.forEach(hookPath => {
    ...
    if (content) {
      console.log(`\\n📄 ${hookPath}`);
      console.log(`   📏 Tamanho: ${content.length} chars`);
      console.log(`   🎯 Função principal: ${content.includes('useUnifiedProperties') ? 'useUnifiedProperties' : 'outro'}`);
...
function analyzePropertiesPanels() {
  ...
  panels.forEach(panelPath => {
    ...
    if (content) {
      console.log(`\\n📋 ${panelPath}`);
      console.log(`   📏 Tamanho: ${content.length} chars`);
      console.log(`   🎯 Hook usado: ${content.includes('useUnifiedProperties') ? 'useUnifiedProperties' : 'outro/nenhum'}`);
