...
export async function validateRegistryCompatibility() {
  ...
  console.log(`📋 Tipos: ${availableTypes.slice(0, 10).join(', ')}${availableTypes.length > 10 ? '...' : ''}\n`);
  ...
  for (let step = 1; step <= 21; step++) {
    ...
    const templatePath = path.join(process.cwd(), 'public', 'templates', `step-${stepId}-template.json`);
        ...
        if (template.blocks && Array.isArray(template.blocks)) {
          template.blocks.forEach(block => {
            ...
            if (availableTypes.includes(blockType)) {
              ...
              results.warnings.push(`Step ${step}: Bloco '${blockType}' não encontrado no registry`);
            }
          });
        }
        
        const status = results.templateStats[step].invalidBlocks === 0 ? '✅' : '⚠️';
        console.log(`${status} Step ${step}: ${results.templateStats[step].validBlocks}/${results.templateStats[step].blocks} blocos válidos`);
  ...
  console.log(`✅ Blocos válidos: ${results.validBlocks} (${(results.validBlocks / results.totalBlocks * 100).toFixed(1)}%)`);
  console.log(`⚠️ Blocos inválidos: ${results.invalidBlocks} (${(results.invalidBlocks / results.totalBlocks * 100).toFixed(1)}%)`);
  ...
  if (results.missingComponents.size > 0) {
    ...
    Array.from(results.missingComponents).forEach(componentType => {
      ...
      console.log(`  • ${componentType}: ${stats.count}x (usado nos steps: ${stats.usedInSteps.join(', ')})`);
    });
  }
  ...
  if (problemTemplates.length > 0) {
    ...
    problemTemplates.forEach(template => {
      console.log(`  • Step ${template.step} (${template.name}): ${template.invalidBlocks} blocos inválidos`);
