...
const COMPONENTS_TO_FIX = [
  // COMPONENTES SEM CONFIGURAÇÃO NO blockDefinitions.ts
  {
    ...
    properties: {
      logoUrl: { type: 'string', label: 'URL do Logo', default: 'https://res.cloudinary.com/dqljyf76t/image/upload/v1744911572/LOGO_DA_MARCA_GISELE_r14oz2.webp' },
  ...
  },
  {
    ...
    properties: {
      ...
      type: { type: 'select', label: 'Tipo', default: 'text', options: [
  ...
  },
  {
    ...
    properties: {
      privacyText: { type: 'string', label: 'Texto de Privacidade', default: 'Política de privacidade' },
      copyrightText: { type: 'string', label: 'Texto de Copyright', default: '© 2025 Todos os direitos reservados' },
      termsText: { type: 'string', label: 'Texto de Termos', default: 'Termos de uso' },
      fontSize: { type: 'select', label: 'Tamanho da Fonte', default: 'text-xs', options: [
        { value: 'text-xs', label: 'Extra Pequeno' },
        { value: 'text-sm', label: 'Pequeno' },
        { value: 'text-base', label: 'Normal' }
      ]},
      textAlign: { type: 'select', label: 'Alinhamento', default: 'center', options: [
  ...
  },
  {
    ...
    properties: {
      ...
      objectFit: { type: 'select', label: 'Ajuste', default: 'cover', options: [
      ...
      alignment: { type: 'select', label: 'Alinhamento', default: 'center', options: [
        { value: 'left', label: 'Esquerda' },
        { value: 'center', label: 'Centro' },
        { value: 'right', label: 'Direita' }
      ]}
  ...
  },
  {
    ...
    properties: {
      ...
      columns: { type: 'select', label: 'Colunas', default: '2', options: [
      ...
      selectionMode: { type: 'select', label: 'Seleção', default: 'single', options: [
      ...
      imagePosition: { type: 'select', label: 'Posição da Imagem', default: 'top', options: [
  ...
  },
  {
    ...
    properties: {
      ...
      layout: { type: 'select', label: 'Layout', default: 'vertical', options: [
  ...
  },
  {
    ...
    properties: {
      ...
      guideImageUrl: { type: 'string', label: 'URL do Guia', default: 'https://res.cloudinary.com/dqljyf76t/image/upload/v1745071344/GUIA_NATURAL_fzp6fc.webp' },
      primaryStyle: { type: 'select', label: 'Estilo Principal', default: 'Natural', options: [
      ...
      layout: { type: 'select', label: 'Layout', default: 'card', options: [
  ...
  },
  {
    ...
    properties: {
      ...
      title: { type: 'string', label: 'Título', default: 'Seu Estilo Predominante' },
      subtitle: { type: 'string', label: 'Subtítulo', default: 'Descubra seu estilo de moda único' },
      ...
      layout: { type: 'select', label: 'Layout', default: 'centered', options: [
...
function updateBlockDefinitions() {
  ...
  const newImports = COMPONENTS_TO_FIX.map(comp => 
    `import ${comp.component} from "${comp.importPath}";`
  ).join('\n');
...
function updateUnifiedProperties() {
  ...
  log(`✅ ${COMPONENTS_TO_FIX.length} componentes adicionados ao useUnifiedProperties.ts`, 'success');
...
function validateAndFormat() {
  ...
  try {
    ...
    execSync('npx prettier --write "src/config/blockDefinitions.ts" "src/hooks/useUnifiedProperties.ts"', { stdio: 'pipe' });
