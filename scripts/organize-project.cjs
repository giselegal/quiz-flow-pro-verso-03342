...
function analyzeDirectory(dir, basePath = "") {
  ...
  items.forEach((item) => {
    ...
    if (
      ...
      else if (relativePath.includes("test") || item.name.includes(".test."))
        fileTypes.tests++;
    }
  });
}
...
const complexityLevel =
  totalFiles > 1000 ? "ALTA" : totalFiles > 500 ? "MÉDIA" : "BAIXA";
...
if (fileTypes.components > 300) {
  recommendations.push(
    "🔧 Considere dividir componentes em subpastas por funcionalidade",
  );
}
