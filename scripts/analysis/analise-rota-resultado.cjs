...
if (conteudos.app) {
  const rotasEncontradas = [
    { rota: '/resultado', regex: /path="\/resultado".*component={ResultPage}/g, desc: 'Rota principal do resultado' },
    { rota: '/test-resultado', regex: /path="\/test-resultado"/g, desc: 'Rota de teste' },
    { rota: 'Importação ResultPage', regex: /import.*ResultPage.*from.*ResultPage/g, desc: 'Import do componente' },
    { rota: 'Router configurado', regex: /import.*Router.*from.*wouter/g, desc: 'Sistema de roteamento' }
  ];
...
if (conteudos.routes) {
  const configRoutes = [
    { config: 'RESULTADO constant', regex: /RESULTADO:\s*['"]\/resultado['"]/g },
...
if (conteudos.app) {
  const rotaMatch = conteudos.app.match(/<Route\s+path="\/resultado"\s+component={ResultPage}\s*\/>/g);
  if (rotaMatch) {
...
if (conteudos.resultPage && conteudos.resultPage.includes('export default ResultPage')) checksPassaram++;

console.log(`📊 Verificações: ${checksPassaram}/${totalChecks} (${Math.round(checksPassaram/totalChecks*100)}%)`);
