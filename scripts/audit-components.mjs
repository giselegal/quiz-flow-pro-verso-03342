...
function analyzeConnectedComponents() {
  ...
  const connectedBlocks = blockMatches
    .map(block => {
...
function generateReport() {
  ...
  console.log('-'.repeat(40));
  console.log(
    `Total: ${unconnected.length} de ${physicalComponents.length} (${Math.round((unconnected.length / physicalComponents.length) * 100)}% não conectados)`
  );
  ...
  console.log('-'.repeat(40));
  console.log(
    `Taxa de Conexão: ${Math.round((connectedBlocks.length / physicalComponents.length) * 100)}%`
  );
