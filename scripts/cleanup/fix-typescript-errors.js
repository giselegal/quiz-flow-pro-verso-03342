...
function fixUseEditorReusableComponents() {
  ...
  // Adicionar cast para BlockType
  content = content.replace('addBlock(componentType);', 'addBlock(componentType as BlockType);');
...
function fixEditorFixedDebug() {
  ...
  const fixes = [
    ...
    [
      "editor?.deleteBlock ? '✅' : '❌'",
      "typeof editor?.deleteBlock === 'function' ? '✅' : '❌'",
    ],
    [
      "editor?.selectBlock ? '✅' : '❌'",
      "typeof editor?.selectBlock === 'function' ? '✅' : '❌'",
    ],
    ["editor?.save ? '✅' : '❌'", "typeof editor?.save === 'function' ? '✅' : '❌'"],
    [
      "editor?.togglePreview ? '✅' : '❌'",
      "typeof editor?.togglePreview === 'function' ? '✅' : '❌'",
    ],
    ['editor.addBlock &&', "typeof editor.addBlock === 'function' &&"],
  ];
