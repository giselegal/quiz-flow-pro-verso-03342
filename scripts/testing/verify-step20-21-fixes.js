...
async function main() {
  ...
  maxScore += 20;
  const routingOk = await checkFileContent(
    'src/App.tsx',
    [
      {
        type: 'contains',
  ...
  maxScore += 25;
  const step20Ok = await checkFileContent(
  ...
  maxScore += 25;
  const mappingOk = await checkFileContent(
  ...
  maxScore += 15;
  const step21Ok = await checkFileContent(
  ...
  maxScore += 15;
  const jsonStep20Ok = await checkFileContent(
  ...
  const jsonStep21Ok = await checkFileContent(
    'src/config/templates/step-21.json',
    [
      {
        type: 'contains',
        value: 'thank-you',
