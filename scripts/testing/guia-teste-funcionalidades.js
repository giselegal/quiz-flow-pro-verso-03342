...
function testarDashboard() {
  ...
  const templates = document.querySelectorAll('[class*="template"], .template, [data-testid*="template"]');
  console.log(`✅ Templates encontrados: ${templates.length}`);
  
  // 3. Procurar botões de ação
  const actionButtons = Array.from(document.querySelectorAll('button')).filter(btn => {
    const text = btn.textContent?.toLowerCase() || '';
    return text.includes('usar template') || 
...
function testarEditor() {
  ...
  const canvas = document.querySelector('[class*="canvas"], [class*="preview"], main, [class*="editor"]');
  ...
  tabs.forEach((tab, i) => {
    console.log(`   ${i + 1}. "${tab.textContent?.trim()}" (ativo: ${tab.getAttribute('data-state') === 'active' || tab.classList.contains('active')})`);
  });
  
  // 5. Verificar componentes arrastáveis
  const draggableComponents = document.querySelectorAll('[draggable="true"], [class*="draggable"], [class*="component"]');
  console.log(`✅ Componentes arrastáveis: ${draggableComponents.length}`);
  
  // 6. Verificar painel de propriedades
  const propertiesPanel = document.querySelector('[class*="properties"], [class*="config"], [class*="settings"]');
  ...
  const componentLabels = Array.from(document.querySelectorAll('span, div, p')).filter(el => {
    ...
    return text.includes('text') || text.includes('button') || text.includes('image') || text.includes('grid');
  });
...
function testar21Etapas() {
  ...
  const pageIndicators = document.querySelectorAll('[class*="page"], [class*="step"], [class*="etapa"]');
  ...
  const deviceControls = document.querySelectorAll('[class*="device"], [class*="mobile"], [class*="tablet"], [class*="desktop"]');
  ...
  const navigationButtons = Array.from(document.querySelectorAll('button')).filter(btn => {
    const text = btn.textContent?.toLowerCase() || '';
    return text.includes('próxima') || text.includes('anterior') || text.includes('next') || text.includes('prev');
  });
...
function testarSalvamento() {
  ...
  const autoSaveIndicators = document.querySelectorAll('[class*="saving"], [class*="saved"], [class*="auto"]');
