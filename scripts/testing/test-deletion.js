...
async function testDeletion() {
  try {
    ...
    const existingBlocks = document.querySelectorAll('[data-block-id], .block-item, .sortable-block');
    ...
    if (existingBlocks.length > 0) {
      ...
      const deleteButtons = document.querySelectorAll('button[title*="xcluir"], button[title*="elete"], .delete-btn, [data-action="delete"]');
      ...
      if (deleteButtons.length > 0) {
        ...
        setTimeout(() => {
          const remainingBlocks = document.querySelectorAll('[data-block-id], .block-item, .sortable-block');
        ...
        allButtons.forEach((btn, index) => {
          ...
          if (text.toLowerCase().includes('excl') || text.toLowerCase().includes('delet') || text.innerHTML.includes('trash')) {
      ...
      const sidebar = document.querySelector('.components-sidebar, .sidebar, [data-testid="components-sidebar"]');
      if (sidebar) {
        console.log('   📋 Sidebar de componentes encontrada');
        
        // Procurar componentes para arrastar
        const draggableComponents = sidebar.querySelectorAll('[draggable="true"], .draggable-component, .component-item');
