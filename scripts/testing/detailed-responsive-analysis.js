...
const responsivePatterns = {
  ...
  "w-\\[\\d+px\\]": {
    ...
    solution:
      "Usar classes responsivas como w-full, w-64, w-72 ou breakpoints sm:w-X md:w-Y",
    severity: "high",
  },
  "min-w-\\[\\d+px\\]": {
    problem: "Largura mínima fixa em pixels",
    solution:
      "Usar classes como min-w-0, min-w-full ou breakpoints responsivos",
    severity: "medium",
  },
  "max-w-\\[\\d+px\\]": {
  ...
  "h-\\[\\d+px\\]": {
    ...
    solution:
      "Usar classes como h-auto, h-screen, h-64 ou definir altura responsiva",
    severity: "high",
  },
  "min-h-\\[\\d+px\\]": {
    problem: "Altura mínima fixa em pixels",
    solution:
      "Usar classes como min-h-screen, min-h-full ou breakpoints responsivos",
    severity: "medium",
  },
  ...
  "grid-cols-\\d+(?!\\s+(?:sm|md|lg|xl|2xl):)": {
    ...
    solution:
      "Adicionar breakpoints como grid-cols-1 md:grid-cols-2 lg:grid-cols-3",
    severity: "high",
  },
  ...
  "\\babsolute\\b": {
    ...
    solution:
      "Verificar se funciona em diferentes tamanhos de tela ou usar responsive positioning",
    severity: "low",
  },
  ...
  "\\bfixed\\b": {
    ...
    solution:
      "Testar em dispositivos móveis e considerar alternatives responsivas",
    severity: "medium",
  },
};
  ...
  try {
    ...
    if (!hasResponsiveClasses) {
      ...
      issues.push({
        ...
        solution:
          "Adicionar classes como sm:, md:, lg: para diferentes tamanhos de tela",
        severity: "high",
        penalty: 30,
      });
    }
...
function generateDetailedReport() {
  ...
  const results = files
    .map(analyzeComponentResponsiveness)
  ...
  console.log(
    `   ✅ Componentes bons (80+): ${goodComponents.length} (${Math.round((goodComponents.length / results.length) * 100)}%)`
  );
  console.log(
    `   ⚠️  Componentes OK (60-79): ${okComponents.length} (${Math.round((okComponents.length / results.length) * 100)}%)`
  );
  console.log(
    `   ❌ Componentes problemáticos (<60): ${badComponents.length} (${Math.round((badComponents.length / results.length) * 100)}%)`
  );
  console.log("\n");
  ...
  results.slice(0, 10).forEach((result, index) => {
    console.log(
      `${index + 1}. ${result.status} ${result.fileName} (Score: ${result.score}/100)`
    );
  ...
  if (urgentComponents.length > 0) {
    console.log(
      "🚨 COMPONENTES QUE PRECISAM DE ATENÇÃO IMEDIATA (Score < 40):\n"
    );
  ...
  console.log(
    "   1. Substituir larguras/alturas fixas em pixels por classes Tailwind responsivas"
  );
  console.log("   2. Usar breakpoints (sm:, md:, lg:) para adaptar layouts");
  console.log(
    "   3. Implementar grid responsivo: grid-cols-1 md:grid-cols-2 lg:grid-cols-3"
  );
  console.log(
    "   4. Usar tamanhos de fonte responsivos: text-sm md:text-base lg:text-lg"
  );
