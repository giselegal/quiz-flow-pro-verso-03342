...
async function testBackendConnection() {
  ...
  try {
    ...
    const updateResponse = await fetch(`http://localhost:3001/api/schema-driven/funnels/${createdFunnel.data.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    const deleteResponse = await fetch(`http://localhost:3001/api/schema-driven/funnels/${createdFunnel.data.id}`, {
      method: 'DELETE'
    });
