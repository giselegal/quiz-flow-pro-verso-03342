...
const checkComponentFiles = () => {
  ...
  console.log(`✅ Encontrados ${inlineFiles.length} arquivos inline:`, inlineFiles.slice(0, 5).join(', '), inlineFiles.length > 5 ? '...' : '');
  
  // Standard blocks
  const blocksDir = './src/components/editor/blocks';
  const blockFiles = fs.readdirSync(blocksDir).filter(f => f.endsWith('.tsx') && !f.includes('Universal') && !f.includes('index'));
  console.log(`✅ Encontrados ${blockFiles.length} arquivos blocks:`, blockFiles.slice(0, 5).join(', '), blockFiles.length > 5 ? '...' : '');
