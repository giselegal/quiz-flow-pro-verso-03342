...
const QuizFunnelEditor: React.FC<QuizFunnelEditorProps> = ({ funnelId, templateId }) => {
    ...
    // ⚠️ DISABLED - BlockRegistry runtime removido na FASE 4
    // Este código usava BlockRegistry.get() que não existe mais
    // useEffect(() => {
                            ...
                            onChange={(e) => {
                                ...
                                reader.onload = (ev) => {
                                    try {
                                        ...
                                        // ⚠️ DISABLED - BlockRegistry validation removed in FASE 4
                                        // try {
                    ...
                    <div className="flex-1 border-r relative bg-muted/10 flex flex-col">
                        ...
                        <div className="flex-1 overflow-auto">
                            {activeBlockPreviewId && selectedStep?.blocks?.length ? (
                                // ⚠️ DISABLED - BlockRegistry preview removed in FASE 4
                                <div className="p-4 text-[11px] text-amber-600">
                                    Preview de bloco individual desabilitado (BlockRegistry removido)
                                </div>
                            ) : (
                                ...
                                <>
                                    {(selectedStep.type === 'result' || selectedStep.type === 'offer') && (
                                        <div className="border rounded p-2 bg-muted/30 space-y-2">
                                            <div className="flex items-center justify-between">
                                                <span className="text-[11px] font-semibold">Blocos (legado desabilitado)</span>
                                                {/* ⚠️ DISABLED - BlockRegistry.list removed in FASE 4 */}
                                            </div>
                                            <div className="text-[10px] text-amber-600 p-2">
                                                Sistema de blocos modulares desabilitado. BlockRegistry foi migrado para UnifiedBlockRegistry.
                                            </div>
                                            {/* Original block list code disabled
                                            <div className="space-y-1">
