...
function checkEditorFixedIntegration() {
  ...
  return {
    ...
    hasCanvasDropZone,
    score: [hasOptimizedImport, hasEditorContext, hasFunnelStagesPanel, hasCanvasDropZone]
...
function checkEditorContextSteps() {
  ...
  return {
    ...
    has21StepsInit,
    hasStepMapping,
    score: [hasOptimizedImport, hasLoadOptimizedSteps, has21StepsInit, hasStepMapping]
      .filter(Boolean).length * 25
  };
}
...
function checkOptimizedConfiguration() {
  ...
  console.log(`  ${hasCorrectStepCount ? '✅' : '❌'} 21 etapas definidas (encontradas: ${uniqueSteps.length})`);
  ...
  if (hasCorrectStepCount) {
    console.log(`  📋 Etapas encontradas: ${uniqueSteps.slice(0, 5).join(', ')}... (total: ${uniqueSteps.length})`);
  }
  ...
  return {
    ...
    hasCalculations,
    hasConversion,
    score: [hasCorrectStepCount, hasQuizData, hasStyles, hasCalculations, hasConversion]
...
function checkFunnelStagesPanel() {
  ...
  return {
    ...
    hasActiveStage,
    hasStageActions,
    score: [hasEditorContext, hasStagesMapping, hasActiveStage, hasStageActions]
      .filter(Boolean).length * 25
  };
}
