...
function testManualSave() {
  ...
  const hasReactState = window.React && window.React.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
...
document.addEventListener('click', (event) => {
  if (event.target && event.target.closest && event.target.closest('button[class*="bg-\\[\\#B89B7A\\]"]')) {
