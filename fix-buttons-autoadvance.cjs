  ...
  try {
    ...
    if (optionsGrid) {
      ...
      if (config.multipleSelection !== undefined)
        props.multipleSelection = config.multipleSelection;
      ...
      if (config.validationMessage !== undefined)
        props.validationMessage = config.validationMessage;
      ...
      console.log(
        `   🔘 Options-Grid: ${config.multipleSelection ? 'múltipla' : 'única'} (${config.minSelections}-${config.maxSelections})`
      );
      console.log(
        `   ⚡ Auto-avanço: ${config.autoAdvance ? `✅ (${config.autoAdvanceDelay}ms)` : '❌ manual'}`
      );
    }
    ...
    if (button) {
      ...
      console.log(
        `   ▶️  Botão: "${config.buttonText}" (${config.enableOnSelection ? 'condicional' : 'sempre ativo'})`
      );
    }
...
function updateTSXTemplate(stepNum, config) {
  ...
  try {
    ...
    if (config.minSelections !== undefined) {
      content = content.replace(/minSelections:\s*\d+/g, `minSelections: ${config.minSelections}`);
    }

    if (config.maxSelections !== undefined) {
      content = content.replace(/maxSelections:\s*\d+/g, `maxSelections: ${config.maxSelections}`);
    }
    ...
    if (config.buttonText) {
      content = content.replace(/text:\s*['"`][^'"`]*['"`]/g, `text: '${config.buttonText}'`);
    }
...
function validateConfiguration() {
  ...
  validationResults.forEach(result => {
    ...
    if (result.optionsGrid) {
      console.log(
        `   🔘 Seleção: ${result.optionsGrid.multipleSelection ? 'múltipla' : 'única'} (${result.optionsGrid.minSelections}-${result.optionsGrid.maxSelections})`
      );
      console.log(`   ⚡ Auto-avanço: ${result.optionsGrid.autoAdvance ? '✅' : '❌'}`);
    }
    if (result.button) {
      console.log(
        `   ▶️  Botão: "${result.button.text}" (${result.button.enableOnSelection ? 'condicional' : 'sempre ativo'})`
      );
    }
    console.log('');
  });
