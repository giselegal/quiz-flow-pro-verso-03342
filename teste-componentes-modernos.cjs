...
const categorias = {
  'Layout': ['flex-container-horizontal', 'flex-container-vertical', 'image-text-card', 'section-divider'],
  'Vendas': ['countdown-timer-real', 'pricing-card-modern', 'cta-button-modern', 'stats-counter', 'testimonial-card', 'feature-highlight'],
...
const caracteristicas = [
  { nome: 'BoxFlex Horizontal', regex: /justifyContent.*space-between|flex.*horizontal/gi },
...
console.log(`📦 Componentes: ${totalEncontrados}/${totalEsperados} (${Math.round(totalEncontrados/totalEsperados*100)}%)`);
console.log(`⚡ Características: ${caracteristicasEncontradas}/${caracteristicas.length} (${Math.round(caracteristicasEncontradas/caracteristicas.length*100)}%)`);
