...
function validateCoreComponents() {
  ...
  coreComponents.forEach(component => {
    ...
    if (fs.existsSync(componentPath)) {
      ...
      if (content.includes('export') && content.includes(component.split('-').map(w => 
...
function validateUnifiedProperties() {
  ...
  try {
    ...
    const checks = [
      ...
      { name: 'Função principal', pattern: /function useUnifiedProperties|const useUnifiedProperties/ },
...
function runPerformanceAnalysis() {
  ...
  console.log(`  ${componentCount <= 50 ? '✅' : '⚠️'} Performance: ${componentCount <= 50 ? 'ÓTIMA' : 'PODE MELHORAR'}`);
...
function generateValidationReport() {
  ...
  console.log(`🎯 STATUS: ${percentage >= 80 ? '✅ EXCELENTE' : percentage >= 60 ? '⚠️ BOM' : '❌ PRECISA MELHORAR'}`);
  ...
  console.log(`  • Unified Properties: ${results.unifiedProperties.valid ? '✅ OK' : '⚠️ REVISAR'}`);
