...
// Tipos locais (em produção, viriam de @/utils/scoreCalculator)
interface Answer {
    questionId: string;
...
// Mocks das funções (em produção, viriam de @/utils/scoreCalculator)
const calculateScore = (answers: Answer[], rules?: any): ScoreResult => {
    // Implementação mock para exemplo
    const totalScore = answers.length * 15;
    return {
        totalScore,
        maxPossibleScore: answers.length * 20,
        percentage: Math.round((totalScore / (answers.length * 20)) * 100),
        ...
        badges: ['🔥 Hot Streak'],
        level: { current: 2, name: 'Aprendiz', nextLevelAt: 250 }
    };
};

const analyzePerformance = (result: ScoreResult, answers: Answer[]): PerformanceAnalysis => {
    return {
        strengths: ['Respostas rápidas'],
        weaknesses: [],
...
export const QuizQuestionWithScoring: React.FC<QuizQuestionProps> = ({
    stepId,
    questionText,
    options,
    weight = 1,
...
export const QuizScoreHeader: React.FC<QuizScoreHeaderProps> = ({
    totalScore,
    level,
    badges,
    currentXP,
    ...
    return (
        <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-6 rounded-lg">
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-2xl font-bold">{totalScore} pts</h3>
                    <p className="text-sm opacity-90">Nível: {level.name}</p>
                </div>
            ...
            <div className="mt-4">
                <div className="bg-white/20 rounded-full h-2">
                    <div
                        ...
                        style={{ width: `${(currentXP / nextLevelAt) * 100}%` }}
                    />
                </div>
                <p className="text-xs mt-1 opacity-75">
                    {currentXP} / {nextLevelAt} XP para próximo nível
                </p>
            </div>
        </div>
    );
