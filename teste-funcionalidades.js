...
function testeDashboardCriacaoFunil() {
  ...
  try {
    ...
    const templateButtons = Array.from(createButtons).filter(btn => 
...
function testeEditorComponentesPropriedades() {
  ...
  try {
    ...
    const components = document.querySelectorAll('[draggable="true"], [class*="component"], [class*="block"]');
...
function teste21EtapasResponsividade() {
  ...
  try {
    ...
    const pagesList = document.querySelectorAll('[class*="page"], [class*="step"], [class*="etapa"]');
    console.log('✅ Etapas/páginas encontradas:', pagesList.length);
    
    // Verificar se há navegação entre etapas
    const navigation = document.querySelectorAll('[class*="nav"], button[class*="page"], [role="button"]');
    ...
    const responsiveElements = document.querySelectorAll('[class*="responsive"], [class*="hidden"], [class*="lg:"], [class*="md:"], [class*="sm:"]');
...
function testeSalvamentoPersistencia() {
  ...
  try {
    ...
    const funnelKeys = localStorageKeys.filter(key => 
      key.includes('funnel') || 
      key.includes('schema') || 
      key.includes('editor')
    );
    ...
    const statusIndicators = document.querySelectorAll('[class*="saving"], [class*="saved"], [class*="status"]');
    ...
    const hasSaveFunction = typeof window.forceSave === 'function' || 
...
function testeFuncionalidadesAvancadas() {
  ...
  try {
    ...
    const undoRedoButtons = Array.from(document.querySelectorAll('button')).filter(btn => {
      const text = btn.textContent?.toLowerCase() || '';
      return text.includes('desfazer') || text.includes('refazer') || text.includes('undo') || text.includes('redo');
    });
